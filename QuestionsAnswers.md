...
+ Measuring the TCB: Using netstat with the ts utility to have a timestamp on every line to see after what time the connection is abandoned with 1 open connection ( command is "netstat -at | grep SYN_RECV |  ts '[%Y-%m-%d %H:%M:%S]'" ). To see how many connections can be upheld, I use the command "netstat -at | grep SYN_RECV | wc -l" to count the lines containing SYN_RECV in netstat. To complete this step the automatic RST packets sent by hping3 have to be prevented. This is done with a firewall rule: "sudo iptables -I OUTPUT 1 -d Destination-Address -p tcp --tcp-flags RST RST -j DROP".
+ The connection is abandoned after about 6 seconds. The maximum amount of connections is 128.
+ After sending 150 SYN packets to the test host, a HTTP connection is still immediately establishable. From this we can infer that on a new connection, the oldest half open connection gets released.

## 2.
+ The IPS was put in between the client and the webserver. This was done on a physical level as well as with the iptables rule "sudo iptables -t nat -A OUTPUT -p tcp --dport 80 -j DNAT --to-destination 127.0.0.1:4567". The IPS listens to port 4567 and forwards all valid packets back to port 80 to a server address, which the admin has to give as argument when starting the IPS.
+ The IPS module against SYN-Flooding keeps track of which source sends how many SYNs. If it has recorded more than 30 SYNs it blocks traffic from that source. Every 30 seconds the module's memory gets wiped clean. Whenever a SYN comes from a source its counter increments, if a ACK is received, its counter decrements. With this simple increment decrement rule, it is insured, that only SYN floods are blocked and not valid connections.
+ This approach does not help if the adversary attacks with a randomized source address. It also allows 30 SYN packets every 30 seconds from each source. With multiple hosts attacking, the server can still be flooded.
+ This approach does not save the IPS from being overloaded. With millions of packets per second it will still slow down legitimate connections to the webserver.

## 3.
+ The described approach is of course generally good. It will work against dumb TCP connections.
...
## 4.
+ I am sending a redirect, but not even a browser is answering. I have checked for correct checksums in TCP, IP4 and Ethernet protocols, for correct sequence and acknowledgement numbers as well as a well formed 302 http packet and a good tcp timestamp. None of this fixes it. I am sorry that I could not complete this question.

## 5.
+ The minimal information that has to be included is anything that uniquely identifies the connection. A possibility is the sequence number together with the acknowledgement number generated by the IPS as well as the source and destination IP addresses. Since anybody can then just send a valid SYN cookie in a ACK without ever sending a SYN, the cookie has to be encrypted. We can do this, by generating a hash of these values together with a private key and a timestamp that changes every 10 seconds. The timestamp makes it hader to brute force the SYN cookie and lets handshakes take up to 10 seconds. For a hash function we should use something fast, like AES since it is often implemented in hardware.
+ Sadly I wasted too much time on q4 and did not start implementing this.
